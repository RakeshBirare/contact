...
//FIXME:TODO:import { useHistory } from "react-router-dom";
...
const AddContact = () => {
  ...
  const imagePicker = async e => {
    ...
    try {
      ...
      .child('image/' + file.name).put(resizedImage, metadata)

      uploadTask.on(
        firebase.storage.TaskEvent.STATE.CHANGED,
        snapshot  => {
          setIsUploading(true)
  ...
  const updateContact = async () => {
    ...
    try {
      firebase
      .database().ref("contacts/" + contactToUpdateKey)
      .set({
        name,
        email,
        phoneNumber,
